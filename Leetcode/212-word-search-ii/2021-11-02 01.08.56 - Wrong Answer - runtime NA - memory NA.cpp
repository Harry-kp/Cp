...
public:
    ...
    bool ok(char c,int i,int j,vector<vector<int>>& vis,vector<vector<char>>& board){
        if(i>=0 and j>=0 and i<board.size() and j<board[0].size() and !vis[i][j] and board[i][j] == c)
            return true;
    return false;
    }
...
//         create trie
        ...
        for(int i = 0;i<n;i++){
            for(int j = 0;j<m;j++){
                ...
                if(trie[0][board[i][j]-'a']!=0)
                dfs(i,j,trie[0][board[i][j]-'a'],vis,ans,board,trie,terminal);
            }
        }
